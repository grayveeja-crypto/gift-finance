...
import { Home, BarChart2, CreditCard, Target, Plus, RefreshCw, ChevronRight, ChevronDown, X, Upload, TrendingUp, Wallet, Bell, Send, RotateCcw, Search } from "lucide-react";
import { AreaChart, Area, PieChart, Pie, Cell, ResponsiveContainer, Tooltip, BarChart, Bar, XAxis, YAxis, LineChart, Line } from "recharts";
...
// ─── FALLBACK DATA ────────────────────────────────────────────────────────────
const FB_H = [
  { code:"SCBRM2",          name:"SCB RMF Thai Equity",    type:"Personal",   cls:"Thai Equity",   value:17023.16,  cost:15000,     nav:15.8149, dailyPct:3.26,  totalPct:13.49, units:1076.40  },
  { code:"SCBRMS&P500",     name:"SCB S&P500",             type:"Personal",   cls:"US Equity",     value:102748.71, cost:100000,    nav:22.1737, dailyPct:5.57,  totalPct:2.75,  units:4633.81  },
  { code:"SCBRMWORLD(A)",   name:"SCB RMF World Equity A", type:"Personal",   cls:"Global Equity", value:51137.14,  cost:50000,     nav:15.9881, dailyPct:3.26,  totalPct:2.27,  units:3198.45  },
  { code:"SCBSFF",          name:"SCB SF Fixed Income",    type:"Personal",   cls:"Fixed Income",  value:112876.64, cost:110000,    nav:21.6987, dailyPct:2.62,  totalPct:2.62,  units:5202.00  },
  { code:"UOB PERMPOONSU",  name:"UOB Permpool Fixed",     type:"Retirement", cls:"Fixed Income",  value:314874.99, cost:293955.41, nav:18.5129, dailyPct:-0.16, totalPct:7.12,  units:17008.41 },
  { code:"UOB SMART GOLD",  name:"UOB Smart Gold Fund",    type:"Retirement", cls:"Gold",          value:124651.87, cost:117582.16, nav:22.9638, dailyPct:10.98, totalPct:6.01,  units:5428.19  },
  { code:"UNITED GLOBAL BA",name:"United Global Balanced", type:"Retirement", cls:"Balanced",      value:313814.98, cost:293955.41, nav:10.6458, dailyPct:0.49,  totalPct:6.76,  units:29477.82 },
  { code:"UNITED GLOBAL DU",name:"United Global Durable",  type:"Retirement", cls:"Global Equity", value:437186.12, cost:411537.58, nav:13.9045, dailyPct:-4.89, totalPct:6.23,  units:31442.06 },
  { code:"UNITED GLOBAL IN",name:"United Global Income",   type:"Retirement", cls:"Fixed Income",  value:62710.91,  cost:58791.09,  nav:10.7678, dailyPct:-0.48, totalPct:6.67,  units:5823.93  },
];
const FB_D = [
...
function extractBI(tab) {
  let budget=70400,income=75400;
  (Array.isArray(tab)?tab.slice(0,5):[]).forEach(r=>{
    const s=(Array.isArray(r)?r:Object.values(r)).map(v=>String(v||"")).join(" ");
    const bm=s.match(/budget[^฿\d]*฿?\s*([\d,]+)/i); const sm=s.match(/salary[^฿\d]*฿?\s*([\d,]+)/i);
    if(bm) budget=pn(bm[1]); if(sm) income=pn(sm[1]);
  });
...
function parseHoldings(raw) {
  ...
  for(const tab of tabs){
    ...
    const p=rows.map(r=>{
      ...
      const value=pn(r["Total Value"]||r["total_value"]||r["Value"]||0);
...
function parseDebts(raw) {
  const tabs=[raw?.["Debts"],raw?.Debts,raw?.debts,Array.isArray(raw)?raw:null].filter(Boolean);
  for(const tab of tabs){
    const rows=s2o(Array.isArray(tab)?tab:[]);
    if(!rows.length) continue;
    if(!Object.keys(rows[0]).some(k=>k.toLowerCase().includes("debt")||k.toLowerCase().includes("balance"))) continue;
    const p=rows.map(r=>({ name:String(r["Debt"]||r["Name"]||"").trim(), balance:pn(r["Balance"]||0), rate:pn(r["Interest"]||r["Rate"]||0), monthly:pn(r["Monthly Payment"]||0), interest:pn(r["Monthly Interest"]||0), principal:pn(r["Principal Payment"]||r["Principal"]||0), years:pn(r["Remaining Years"]||r["Years"]||0) })).filter(d=>d.name&&d.balance>0);
    if(p.length) return p;
  }
  return null;
}
function parseTarget(raw) {
  const tabs=[raw?.["TargetAllocation"],raw?.TargetAllocation,Array.isArray(raw)?raw:null].filter(Boolean);
  for(const tab of tabs){
    const rows=s2o(Array.isArray(tab)?tab:[]);
    if(!rows.length) continue;
    if(!Object.keys(rows[0]).some(k=>k.toLowerCase().includes("target")||k.toLowerCase().includes("asset"))) continue;
...
function parseHistory(raw) {
  const tabs=[raw?.["History"],raw?.History,Array.isArray(raw)?raw:null].filter(Boolean);
  for(const tab of tabs){
    const rows=s2o(Array.isArray(tab)?tab:[]);
    if(!rows.length) continue;
    if(!Object.keys(rows[0]).some(k=>k.toLowerCase().includes("portfolio")||k.toLowerCase().includes("date"))) continue;
    const p=rows.filter(r=>r.Date||r.date).map(r=>({ m:String(r.Date||r.date).slice(0,7), portfolio:pn(r["Portfolio Value"]||r["Portfolio"]||0), debt:pn(r["Total Debt"]||r["Debt"]||0), nw:pn(r["Net Worth"]||0) })).filter(h=>h.portfolio>0);
    if(p.length) return p;
  }
  return null;
}
function parseCF(raw) {
  const tabs=[raw?.["Cash Flow"],raw?.CashFlow,Array.isArray(raw)?raw:null].filter(Boolean);
  for(const tab of tabs){
    const rows=s2o(Array.isArray(tab)?tab:[]);
    if(!rows.length) continue;
...
function Spark({data,color,h=26}){
  ...
  return(<ResponsiveContainer width="100%" height={h}><AreaChart data={data} margin={{top:1,bottom:1,left:0,right:0}}><defs><linearGradient id={id} x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={color} stopOpacity={0.2}/><stop offset="95%" stopColor={color} stopOpacity={0}/></linearGradient></defs><Area type="monotone" dataKey="v" stroke={color} strokeWidth={1.8} fill={`url(#${id})`} dot={false}/></AreaChart></ResponsiveContainer>);
}

// ─── COUNTER ─────────────────────────────────────────────────────────────────
function Counter({to,dur=900}){
  const [v,setV]=useState(0); const raf=useRef();
  useEffect(()=>{const t0=performance.now();const tick=now=>{const p=Math.min((now-t0)/dur,1),e=1-Math.pow(1-p,3);setV(to*e);if(p<1)raf.current=requestAnimationFrame(tick);};raf.current=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf.current);},[to]);
...
function CTip({active,payload,label}){
  ...
  return(<div style={{background:"#0C1120",border:`1px solid ${T.border}`,borderRadius:10,padding:"8px 12px",fontSize:11}}><div style={{color:T.muted,marginBottom:3,fontWeight:600}}>{label}</div>{payload.map((p,i)=><div key={i} style={{color:p.color,fontWeight:700,fontFamily:T.mono}}>฿{Math.round(p.value).toLocaleString()}</div>)}</div>);
}
      ...
      <div style={{position:"absolute",top:0,right:0,width:280,height:"100%",background:"#0A0E1A",borderLeft:`1px solid ${T.border}`,padding:24,animation:"slideIn .25s cubic-bezier(.16,1,.3,1)"}}>
        ...
        <div style={{textAlign:"center",marginBottom:24,marginTop:16}}>
          <label style={{cursor:"pointer",display:"block"}}>
            <div style={{width:80,height:80,borderRadius:"50%",margin:"0 auto 10px",overflow:"hidden",border:`2px solid ${T.accent}`,display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#6366F1,#F472B6)",fontSize:28,color:"white",position:"relative"}}>
              {photo?<img src={photo} style={{width:"100%",height:"100%",objectFit:"cover"}} alt="profile"/>:"G"}
              <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",opacity:0,transition:"opacity .2s"}} className="photo-overlay"><Upload size={18} color="white"/></div>
            </div>
        ...
        {[{label:"Currency",val:"฿ Thai Baht"},{label:"Last updated",val:"May 11, 2026"},{label:"Data source",val:"Google Sheets"}].map((r,i)=>(
    ...
    <div style={{position:"fixed",inset:0,zIndex:250,display:"flex",justifyContent:"flex-end"}}>
      ...
      <div style={{position:"relative",width:300,height:"100%",background:"#0A0E1A",borderLeft:`1px solid ${T.border}`,padding:22,overflowY:"auto",animation:"slideIn .28s cubic-bezier(.16,1,.3,1)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
          {[{l:"Daily",v:`${sgn(fund.dailyPct)}${fd(fund.dailyPct)}%`,c:clr(fund.dailyPct)},{l:"Total Return",v:`${sgn(fund.totalPct)}${fd(fund.totalPct)}%`,c:clr(fund.totalPct)},{l:"Gain / Loss",v:`${sgn(gl)}${fmt(gl)}`,c:clr(gl)},{l:"Cost Basis",v:fmt(fund.cost),c:T.text2},{l:"NAV",v:`฿${fund.nav.toFixed(4)}`,c:T.text2},{l:"Units",v:fund.units.toFixed(2),c:T.text2}].map((s,i)=>(
        ...
        <div style={{display:"flex",gap:8}}>
          <button style={{flex:1,padding:11,borderRadius:12,fontWeight:700,fontSize:12,background:"linear-gradient(135deg,#22C55E,#16A34A)",border:"none",color:"white",cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>Buy More</button>
          <button style={{flex:1,padding:11,borderRadius:12,fontWeight:700,fontSize:12,background:"transparent",border:`1px solid ${T.border}`,color:T.muted,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>Sell</button>
        </div>
      </div>
    </div>
  );
...
// ─── AI PANEL ────────────────────────────────────────────────────────────────
const QUICK=["How is my portfolio doing?","Should I rebalance?","Am I on track for retirement?","Review my May spending"];
function AIPanel({open,onClose,holdings,debts,spendingMonths}){
  const [msgs,setMsgs]=useState([{r:"a",t:"Hi Gift! I'm your AI advisor with access to your real portfolio data. Ask me anything 📊"}]);
  ...
  const ctx=`Gift's Portfolio: Total ${fmt(total)}, Net Worth ${fmt(total-debt)}, Debt ${fmt(debt)}. Holdings: ${holdings.map(h=>`${h.code}(${h.cls}) ${fmt(h.value)} ${sgn(h.dailyPct)}${fd(h.dailyPct)}% daily`).join("; ")}. Latest spend (${latest?.m}): ${fmt(latest?.spent||0)} vs budget ${fmt(latest?.budget||70400)}.`;
  const send=async text=>{
    const msg=text||inp; if(!msg.trim()||busy) return;
    const nm=[...msgs,{r:"u",t:msg}]; setMsgs(nm); setInp(""); setBusy(true);
    try{const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:700,system:`You are Gift's personal AI financial advisor. Be concise and warm. Use Thai Baht (฿). Keep replies under 100 words.\n\n${ctx}`,messages:nm.map(m=>({role:m.r==="u"?"user":"assistant",content:m.t}))})});const d=await res.json();setMsgs(p=>[...p,{r:"a",t:d.content?.[0]?.text||"Try again."}]);}catch{setMsgs(p=>[...p,{r:"a",t:"Connection error."}]);}
    setBusy(false);
  };
  if(!open) return null;
  return(
    <div style={{position:"fixed",inset:0,zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"flex-end",padding:12,pointerEvents:"none"}}>
      <div style={{width:310,height:490,background:"#0A0E1A",border:`1px solid ${T.border}`,borderRadius:22,display:"flex",flexDirection:"column",boxShadow:"0 32px 80px rgba(0,0,0,0.5)",overflow:"hidden",pointerEvents:"auto",animation:"slideUp .25s cubic-bezier(.16,1,.3,1)"}}>
        <div style={{padding:"11px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:8}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:12,fontWeight:700,color:T.text}}>AI Advisor</div><div style={{fontSize:9,color:T.green}}>● Connected to your data</div></div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"10px 12px",display:"flex",flexDirection:"column",gap:8}}>
          ...
          {busy&&<div style={{display:"flex",gap:6}}><div style={{width:18,height:18,borderRadius:6,background:"linear-gradient(135deg,#6366F1,#38BDF8)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,flexShrink:0}}>✦</div><div style={{padding:"8px 12px",borderRadius:"4px 14px 14px 14px",background:T.surf,display:"flex",gap:4,alignItems:"center"}}>{[0,1,2].map(i=><div key={i} style={{width:5,height:5,borderRadius:"50%",background:T.accent,opacity:.7,animation:`bounce .9s ${i*.15}s ease-in-out infinite`}}/>)}</div></div>}
          <div ref={end}/>
        </div>
        {msgs.length<=1&&<div style={{padding:"0 12px 8px"}}>{QUICK.map((q,i)=><button key={i} onClick={()=>send(q)} style={{display:"block",width:"100%",textAlign:"left",background:"rgba(99,102,241,0.07)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:9,padding:"6px 10px",color:"#C7D2FE",fontSize:11,cursor:"pointer",marginBottom:4,fontFamily:"'Inter',sans-serif"}}>{q} →</button>)}</div>}
        <div style={{padding:"8px 12px",borderTop:`1px solid ${T.border}`,display:"flex",gap:7}}>
          <input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Ask anything…" style={{flex:1,background:"rgba(255,255,255,0.05)",border:`1px solid ${T.border}`,borderRadius:10,padding:"8px 11px",fontSize:12,color:T.text,outline:"none",fontFamily:"'Inter',sans-serif"}}/>
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:400}}>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,maxHeight:"70vh",background:"#0A0E1A",borderRadius:"20px 20px 0 0",padding:20,overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><div style={{fontSize:14,fontWeight:800,color:T.text}}>🔍 API Inspector</div><button onClick={onClose} style={{background:"transparent",border:`1px solid ${T.border}`,borderRadius:8,padding:"4px 10px",color:T.muted,cursor:"pointer",fontSize:11}}>Close</button></div>
        <div style={{fontSize:11,color:T.gold,background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:10,padding:"8px 12px",marginBottom:14}}>If both show errors, open the debug inspector in your browser console after deploying to a real server.</div>
        {[{title:"📊 Portfolio API",color:"#818CF8",err:portErr,raw:portRaw},{title:"💰 Spending API",color:"#38BDF8",err:spendErr,raw:spendRaw}].map((s,i)=>(
          <div key={i} style={{marginBottom:16}}>
            ...
            {s.err?<div style={{color:T.red,fontSize:11,padding:10,background:"rgba(248,113,113,0.08)",borderRadius:10,fontFamily:T.mono}}>{s.err}</div>:<pre style={{fontSize:9,color:T.muted,background:"rgba(255,255,255,0.03)",borderRadius:10,padding:12,overflowX:"auto",whiteSpace:"pre-wrap",wordBreak:"break-all",maxHeight:160,overflowY:"auto",margin:0}}>{s.raw===null?"⏳ Loading…":s.raw===undefined?"No data":JSON.stringify(s.raw,null,2)}</pre>}
          </div>
        ))}
      </div>
    </div>
...
export default function App(){
  ...
  const [aSlice,setASlice]=useState(null);
  const [expandDebt,setExpandDebt]=useState(null);
  const [selMonth,setSelMonth]=useState(0);

  const fetchAll=useCallback(async(silent=false)=>{
    if(!silent) setLoading(true); setRefreshing(true); let live=false;
    try{const res=await fetch(API_PORT,{mode:"cors"}); if(!res.ok) throw new Error(`HTTP ${res.status}`); const raw=await res.json(); setPortRaw(raw); setPortErr(null);
  ...
  const sparkData=(base)=>Array.from({length:5},(_,i)=>({v:base*(0.91+i*.023)}));

  if(loading) return(
    <div style={{fontFamily:"'Inter',sans-serif",background:T.bg,color:T.text,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:12}}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <div style={{width:46,height:46,borderRadius:14,background:"linear-gradient(135deg,#6366F1,#38BDF8)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:900,animation:"spin 2s linear infinite"}}>G</div>
      <div style={{fontSize:13,color:T.muted}}>Connecting to your sheets…</div>
    </div>
  );
  ...
  return(
    <div style={{fontFamily:"'Inter','DM Sans',sans-serif",background:T.bg,color:T.text,minHeight:"100vh",maxWidth:480,margin:"0 auto",position:"relative",overflowX:"hidden",WebkitFontSmoothing:"antialiased"}}>
      <style>{`
        ...
        .kc{transition:transform .18s;}.kc:hover{transform:translateY(-2px);}
        *{box-sizing:border-box;}
        ::-webkit-scrollbar{width:2px;height:2px;}::-webkit-scrollbar-thumb{background:rgba(99,102,241,0.3);border-radius:99px;}
        input::placeholder{color:#6B7280;}
        .photo-overlay:hover{opacity:1!important;}
        .nav-item{transition:color .15s;}
      `}</style>

      {/* ── HEADER ── */}
      <header style={{position:"sticky",top:0,zIndex:100,background:"rgba(6,9,18,0.95)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${T.border}`,padding:"0 16px",height:54,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontWeight:800,fontSize:15,letterSpacing:"-.3px",color:T.text}}>Gift Finance</span>
          <button onClick={()=>setDebugOpen(true)} style={{fontSize:9,fontWeight:700,padding:"2px 8px",borderRadius:999,cursor:"pointer",border:"none",color:isLive?"#22C55E":"#FBBF24",background:isLive?"rgba(34,197,94,0.12)":"rgba(251,191,36,0.12)"}}>{isLive?"● Live":"◌ Fallback"}</button>
        </div>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:17,fontWeight:900,letterSpacing:"-1px",color:T.text,fontFamily:T.mono}}>{fmt(TOTAL)}</div>
          <div style={{fontSize:9,color:T.muted}}>{lastUp?`Updated ${lastUp.toLocaleTimeString("th-TH",{hour:"2-digit",minute:"2-digit"})}`:"May 2026"}</div>
        </div>
      ...
      {/* ── TAB NAV ── */}
      <nav style={{position:"sticky",top:54,zIndex:90,background:"rgba(6,9,18,0.95)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${T.border}`,display:"flex",overflowX:"auto",scrollbarWidth:"none",padding:"0 4px"}}>
        {TABS.map(({id,label,Icon})=>(
          <button key={id} onClick={()=>setTab(id)} style={{flex:"0 0 auto",padding:"0 18px",height:43,background:"transparent",border:"none",cursor:"pointer",color:tab===id?T.text:T.inactive,fontWeight:tab===id?700:500,fontSize:12,position:"relative",display:"flex",alignItems:"center",gap:5,transition:"color .18s",whiteSpace:"nowrap",fontFamily:"'Inter',sans-serif",letterSpacing:"-.1px"}}>
            <Icon size={14} style={{opacity:tab===id?1:.7}}/>
            {label}
      ...
      {/* ── CONTENT ── */}
      <main className="tc" style={{padding:"14px 13px 90px",maxWidth:480,margin:"0 auto"}}>

        {/* ══ OVERVIEW ══ */}
        {tab==="overview"&&(<div style={{display:"flex",flexDirection:"column",gap:11}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
            {[
              ...
              <div key={i} className="kc" style={{...card}}>
                ...
                {k.custom
                  ?<div style={{fontSize:22,fontWeight:900,color:T.green,letterSpacing:"-.5px",fontFamily:T.mono,marginBottom:2}}>{k.custom}</div>
                  :<div style={{fontSize:18,fontWeight:800,color:T.text,letterSpacing:"-.5px",fontFamily:T.mono,marginBottom:2}}><Counter to={k.val}/></div>
                }
                <Spark data={sparkData(k.val||100)} color={k.sc} h={22}/>
                <div style={{fontSize:9,fontWeight:600,color:k.c,marginTop:4}}>{k.sub}</div>
              </div>
            ))}
          </div>
          ...
          <div style={card}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              <div>
                <div style={{fontSize:12,fontWeight:700,color:T.text,marginBottom:1}}>Portfolio Growth</div>
                <div style={{fontSize:10,color:T.muted}}>{history.length} data points</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:13,fontWeight:800,color:T.green,fontFamily:T.mono}}>{fmt(TOTAL)}</div>
                <div style={{fontSize:9,color:T.muted}}>Current value</div>
              </div>
            </div>
            <ResponsiveContainer width="100%" height={120}>
              <AreaChart data={history.filter(h=>h.portfolio>0)}>
                <defs><linearGradient id="pgr" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#6366F1" stopOpacity={0.2}/><stop offset="100%" stopColor="#6366F1" stopOpacity={0}/></linearGradient></defs>
                <XAxis dataKey="m" tick={{fill:T.muted,fontSize:9,fontFamily:"'Inter',sans-serif"}} axisLine={false} tickLine={false}/>
                <YAxis tick={{fill:T.muted,fontSize:8}} axisLine={false} tickLine={false} tickFormatter={v=>`฿${(v/1e6).toFixed(1)}M`} width={40}/>
                <Tooltip content={<CTip/>}/>
                <Area type="monotone" dataKey="portfolio" stroke="#818CF8" strokeWidth={2} fill="url(#pgr)" dot={false} activeDot={{r:4,fill:"#818CF8"}}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          {/* Allocation */}
          <div style={card}>
            <div style={{fontSize:12,fontWeight:700,color:T.text,marginBottom:12}}>Asset Allocation</div>
            {ALLOC.map((a,i)=>(
              <div key={i} onMouseEnter={()=>setASlice(i)} onMouseLeave={()=>setASlice(null)}
                style={{display:"flex",alignItems:"center",gap:9,marginBottom:i<ALLOC.length-1?9:0,opacity:aSlice===null||aSlice===i?1:.3,transition:"opacity .2s",cursor:"pointer"}}>
                <div style={{width:7,height:7,borderRadius:"50%",background:a.color,flexShrink:0}}/>
                <div style={{fontSize:11,color:T.text2,flex:1,fontWeight:500}}>{a.cls}</div>
                <div style={{flex:2,height:4,background:"rgba(255,255,255,0.07)",borderRadius:999,overflow:"hidden"}}><div style={{height:"100%",width:`${a.pct*2}%`,background:a.color,borderRadius:999,transition:"width 1.2s ease"}}/></div>
                <div style={{fontSize:11,fontWeight:700,color:T.text,fontFamily:T.mono,minWidth:36,textAlign:"right"}}>{a.pct}%</div>
              </div>
            ))}
          </div>
          ...
          {/* Movers */}
          <div style={card}>
            <div style={{fontSize:12,fontWeight:700,color:T.text,marginBottom:12}}>Today's Movers</div>
            {[...holdings].sort((a,b)=>Math.abs(b.dailyPct)-Math.abs(a.dailyPct)).slice(0,4).map((h,i,arr)=>(
              <div key={i} onClick={()=>setSelFund(h)} className="hrow" style={{display:"flex",alignItems:"center",gap:10,padding:"9px 6px",borderBottom:i<arr.length-1?`1px solid ${T.border}`:"none"}}>
                <div style={{width:32,height:32,borderRadius:10,background:`${CLS_COLOR[h.cls]||T.accent}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>
                  {h.cls==="Gold"?"🥇":h.cls==="US Equity"?"🇺🇸":h.cls==="Thai Equity"?"🇹🇭":h.cls==="Fixed Income"?"🏦":h.cls==="Balanced"?"⚖️":"🌍"}
                </div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:12,fontWeight:700,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.code}</div>
          ...
          <div style={card}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}>
              <div style={{fontSize:12,fontWeight:700,color:T.text}}>AI Insights</div>
            ...
            {[
              ...
              <div key={i} style={{display:"flex",gap:9,padding:"9px 11px",borderRadius:11,marginBottom:i<3?7:0,background:ins.type==="warn"?"rgba(251,191,36,0.07)":ins.type==="ok"?"rgba(74,222,128,0.06)":`${T.accent}0A`,border:`1px solid ${ins.type==="warn"?"rgba(251,191,36,0.18)":ins.type==="ok"?"rgba(74,222,128,0.14)":`rgba(99,102,241,0.18)`}`}}>
                ...
                <span style={{fontSize:11,color:T.text2,lineHeight:1.55,fontWeight:500}}>{ins.t}</span>
              </div>
            ))}
          </div>
        </div>)}
        ...
        {tab==="investments"&&(<div style={{display:"flex",flexDirection:"column",gap:11}}>
          {/* Personal / Retirement split */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
            {[{label:"Personal",val:PERSONAL,pct:+(PERSONAL/TOTAL*100).toFixed(1),c:T.accent},{label:"Retirement",val:RETIREMENT,pct:+(RETIREMENT/TOTAL*100).toFixed(1),c:T.accent2}].map((s,i)=>(
              <div key={i} style={card}>
                ...
                <div style={{fontSize:18,fontWeight:800,color:T.text,fontFamily:T.mono,marginBottom:8,letterSpacing:"-.5px"}}>{fmt(s.val)}</div>
          ...
          <div style={card}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
              <div style={{fontSize:12,fontWeight:700,color:T.text}}>Holdings <span style={{fontSize:10,color:T.muted,fontWeight:400}}>· tap for details</span></div>
              <div style={{position:"relative"}}>
                <Search size={11} style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",color:T.muted,pointerEvents:"none"}}/>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search…" style={{background:"rgba(255,255,255,0.05)",border:`1px solid ${T.border}`,borderRadius:9,padding:"5px 10px 5px 26px",fontSize:11,color:T.text,outline:"none",width:120,fontFamily:"'Inter',sans-serif"}}/>
              </div>
            </div>
            <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:10}}>
              {CLASSES.map(c=>(
                <button key={c} onClick={()=>setFCls(c)} style={{padding:"3px 10px",borderRadius:999,fontSize:10,fontWeight:600,cursor:"pointer",border:`1px solid ${fCls===c?T.accent:T.border}`,background:fCls===c?T.accent:"transparent",color:fCls===c?"white":T.inactive,transition:"all .18s",fontFamily:"'Inter',sans-serif"}}>{c}</button>
              ))}
            </div>
            {FILTERED.map((h,i)=>{
              const daily=h.dailyPct,total=h.totalPct;
              return(
                <div key={i} className="hrow" onClick={()=>setSelFund(h)} style={{display:"flex",alignItems:"center",gap:9,padding:"10px 7px",borderBottom:i<FILTERED.length-1?`1px solid ${T.border}`:"none"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:12,fontWeight:700,color:T.text}}>{h.code}</div>
                    <div style={{fontSize:9,color:T.muted}}>{h.cls} · {h.type}</div>
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
                    <div style={{fontSize:12,fontWeight:700,color:T.text,fontFamily:T.mono}}>{fmt(h.value)}</div>
                    <div style={{fontSize:9,fontFamily:T.mono}}>
                      <span style={{color:clr(total)}}>{sgn(total)}{fd(total)}%</span>
                      <span style={{color:T.dim,margin:"0 4px"}}>·</span>
                      <span style={{color:clr(daily)}}>{sgn(daily)}{fd(daily)}%</span>
                    </div>
                  </div>
          ...
          {/* Target vs Actual */}
          <div style={card}>
            <div style={{fontSize:12,fontWeight:700,color:T.text,marginBottom:4}}>Target vs Actual</div>
            <div style={{fontSize:10,color:T.muted,marginBottom:12}}>Based on your TargetAllocation sheet</div>
            {REBAL.map((r,i)=>{
                ...
                <div key={i} style={{marginBottom:11}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    ...
                    <div style={{display:"flex",gap:7,alignItems:"center"}}>
                      ...
                      <span style={{fontSize:10,fontWeight:700,color:T.text,fontFamily:T.mono}}>{r.actualPct}%</span>
        ...
        {tab==="spending"&&(<div style={{display:"flex",flexDirection:"column",gap:11}}>
          ...
          <div style={card}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
              <span style={{fontSize:11,fontWeight:700,color:T.text}}>Budget Usage</span>
          ...
          {TXNS.length>0&&(
            <div style={card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:13}}>
                <div style={{fontSize:12,fontWeight:700,color:T.text}}>Transactions</div>
              ...
              {spendGroups().map((g,gi)=>{
                ...
                const bgC=isSav?"rgba(74,222,128,0.05)":isNot?"rgba(251,191,36,0.04)":"transparent";
                const bdC=isSav?"rgba(74,222,128,0.15)":isNot?"rgba(251,191,36,0.15)":T.border;
                ...
                return(
                  <div key={gi} style={{marginBottom:5}}>
                    <div onClick={()=>{if(!multi)return;const el=document.getElementById(`tg${gi}`);if(el)el.style.display=el.style.display==="none"?"flex":"none";}}
                      style={{display:"flex",alignItems:"center",gap:9,padding:"9px 8px",borderRadius:12,background:bgC,border:`1px solid ${bdC}`,cursor:multi?"pointer":"default",transition:"background .12s"}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                          <span style={{fontSize:12,fontWeight:700,color:T.text}}>{g.cat}</span>
                      ...
                      <div id={`tg${gi}`} style={{display:"none",flexDirection:"column",paddingLeft:12,marginTop:3,gap:2}}>
                        {g.txns.map((t,ti)=>(
                          <div key={ti} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 9px",borderRadius:9,background:"rgba(255,255,255,0.02)",border:`1px solid ${T.border}`}}>
                            <div>
                              <div style={{fontSize:11,fontWeight:500,color:T.text2}}>{t.desc||t.cat}</div>
                              <div style={{fontSize:9,color:T.muted}}>{t.date}{t.method&&` · ${t.method}`}</div>
                            </div>
              ...
              <div style={{marginTop:11,padding:"10px 12px",background:`${T.accent}08`,borderRadius:11,border:`1px solid ${T.accent}20`}}>
                {(()=>{
                  ...
                  return(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,textAlign:"center"}}>
                    {[{l:"Savings",v:sav,c:T.green},{l:"Fixed",v:fix,c:T.inactive},{l:"Lifestyle",v:disc,c:T.accent2}].map((s,i)=>(
        ...
        {tab==="planning"&&(<div style={{display:"flex",flexDirection:"column",gap:11}}>
          ...
          <div style={{...card,background:"linear-gradient(135deg,rgba(99,102,241,0.12),rgba(56,189,248,0.07))"}}>
            ...
            <div style={{height:5,background:"rgba(255,255,255,0.07)",borderRadius:999,marginTop:14,overflow:"hidden"}}>
              <div style={{height:"100%",width:"78%",background:"linear-gradient(90deg,#6366F1,#38BDF8)",borderRadius:999}}/>
            </div>
          </div>

          {/* Debts */}
          <div style={card}>
            <div style={{fontSize:12,fontWeight:700,color:T.text,marginBottom:12}}>Debt Breakdown</div>
            {debts.map((d,i)=>{
              const open=expandDebt===i;
              return(<div key={i} style={{marginBottom:i<debts.length-1?8:0}}>
                <div onClick={()=>setExpandDebt(open?null:i)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",padding:"9px 0",borderBottom:`1px solid ${T.border}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:9}}>
                    ...
                    <div><div style={{fontSize:12,fontWeight:700,color:T.text}}>{d.name}</div><div style={{fontSize:9,color:T.muted}}>{d.rate}% · {d.years}yr left</div></div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:7}}>
                    <div style={{textAlign:"right"}}><div style={{fontSize:12,fontWeight:800,color:T.red,fontFamily:T.mono}}>{fmt(d.balance)}</div><div style={{fontSize:9,color:T.muted}}>{fmt(d.monthly)}/mo</div></div>
                    <ChevronRight size={13} color={T.dim} style={{transform:open?"rotate(90deg)":"none",transition:"transform .2s"}}/>
                  </div>
                </div>
                {open&&<div style={{padding:"10px 0",borderBottom:`1px solid ${T.border}`,display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,animation:"fadeIn .2s ease-out"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
            ...
            <div style={card}>
              ...
              <div style={{borderTop:`1px solid ${T.border}`,paddingTop:8,display:"flex",justifyContent:"space-between"}}>
                <span style={{fontSize:11,fontWeight:700,color:T.text}}>Net</span>
              ...
              {cashFlow.unallocatedPct>0&&<div style={{fontSize:9,color:T.gold,marginTop:5,fontWeight:600}}>{fd(cashFlow.unallocatedPct,1)}% unallocated</div>}
            </div>
          </div>
          ...
          <div style={card}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div style={{fontSize:12,fontWeight:700,color:T.text}}>Rebalancing</div>
              <button onClick={()=>setAiOpen(true)} style={{fontSize:9,fontWeight:700,color:T.accent,background:`${T.accent}12`,border:`1px solid rgba(99,102,241,0.2)`,borderRadius:7,padding:"3px 9px",cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>Ask AI ✦</button>
            </div>
        ...
        {[
          ...
          <button key={n.id} onClick={()=>n.special?setDebugOpen(true):setTab(n.id)}
            style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",
              justifyContent:n.special?"flex-end":"flex-end",
              gap:3,paddingBottom:n.special?0:0,
              background:"transparent",border:"none",cursor:"pointer",
              color:tab===n.id?"#818CF8":T.inactive,
              fontFamily:"'Inter',sans-serif",transition:"color .15s",
              position:"relative",paddingTop:n.special?0:10}}>
            {n.special
              ? <div style={{width:46,height:46,borderRadius:"50%",background:"linear-gradient(135deg,#6366F1,#38BDF8)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 24px rgba(99,102,241,0.6)",marginBottom:0,transform:"translateY(-10px)"}}>
                  <Plus size={20} color="white"/>
                </div>
              : <>
                  <n.Icon size={17} color={tab===n.id?"#818CF8":T.inactive}/>
                  <span style={{fontSize:9,fontWeight:700,letterSpacing:".01em"}}>{n.label}</span>
                </>
            }
          </button>
        ))}
